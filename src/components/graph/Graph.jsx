...
export default class Graph extends React.Component {
   ...
   * @returns {undefined}
   */
  setNodeHighlightedValue = (id, value = false) =>
    this.tick (
  ...
  onClickNode = clickedNodeId => {
    ...
    } else if (this.props.onClickNode) {
  ...
  resetNodesPositions = () => {
    if (!this.state.config.staticGraph) {
      for (let nodeId of Object.keys (this.state.nodes)) {
  ...
  componentWillReceiveProps (nextProps) {
    ...
    const transform = newConfig.panAndZoom !== this.state.config.panAndZoom
      ? 1
